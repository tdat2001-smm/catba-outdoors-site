...
(function () {
  ...
  const NAV = [
    ...
    { id: 'users',     label: 'Người dùng',        href: 'users.html',     icon: iconShield,    group: 'Hệ thống' },
    { id: 'settings',  label: 'Cài đặt',           href: 'settings.html',  icon: iconCog,       group: 'Hệ thống' }
  ];
  ...
  function renderLayout(opts) {
    const session = Auth.requirePage(opts.active);
    if (!session) return null;
    if (window.Store && Store.seed) Store.seed();

    const active = opts.active;
    const role = Auth.ROLES[session.role] || Auth.ROLES.staff;
    const allowedNav = NAV.filter((n) => role.pages.includes(n.id));
      ...
      <div class="app">
        <aside class="sidebar" id="sidebar">
          ...
          <div class="sidebar-foot">
            <a href="settings.html" class="avatar" title="Cài đặt" style="text-decoration:none">${initials(session.name)}</a>
            <div class="who">${session.name}<small>${role.label}</small></div>
